...
function AddSupportQuestion({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (values) => {
    ...
    setError(null);
    try {
      await AddNewSupportQuestion(values.question, values.answer);
          ...
          >
            <Form className="w-350 ms-3 mt-3 flex flex-col gap-3">
              ...
              <Field
                ...
                className={`w-full h-20 p-3 border-2 rounded-md outline-none transition-all duration-200 placeholder:text-14 focus:border-primary placeholder:text-gray-400`}
              />
              <button
                type={"submit"}
