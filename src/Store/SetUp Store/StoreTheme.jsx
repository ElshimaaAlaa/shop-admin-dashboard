...
function ThemeStore() {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      console.error(error.message || "Failed to save theme configuration");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-8 md:p-16 bg-gradient-to-r from-customBlue-mediumBlue via-customOrange-mediumOrange to-customOrange-mediumOrange min-h-screen flex items-center justify-center">
      <div className="bg-white rounded-md px-6 py-5 flex flex-col  w-full max-w-2xl">
        <div className="flex justify-center my-5">
          <img src="/assets/svgs/vertex.svg" alt="logo" className="w-28" />
        </div>
        <div className="flex items-center gap-3">
          <div className="rounded-full border-primary border-[4px] p-2">
            1/3
          </div>
          <h3 className="text-15 font-bold">Let’s Get Started To Set Up Your Own Store . </h3>
        </div>
        <Formik
        ...
        >
          {({ setFieldValue, values, errors, touched }) => (
            <Form className="w-full">
              <div className="mb-6">
                <h3 className="font-bold text-15 my-3">Upload Logo File</h3>
                <LogoUpload
                  name="theme_image"
              ...
              <BannerUpload
                ...
                bannerPreviews={values.bannerPreviews}
                setFieldValue={setFieldValue}
                errors={errors}
                touched={touched}
              />

              <h3 className="font-bold text-15 mt-5 mb-2">
              ...
              <div className="flex gap-3 mb-6">
                <Field
                  ...
                  className={`w-full bg-gray-100 p-3 border-2 rounded-md outline-none transition-all duration-200 placeholder:text-14 placeholder:text-gray-500 focus:border-primary ${
                ...
                <Field
                  ...
                  className={`w-full p-3 border-2 bg-gray-100 rounded-md outline-none transition-all duration-200 placeholder:text-14 placeholder:text-gray-500 focus:border-primary ${
              ...
              <div className="flex justify-end mt-5">
                <button
                  type="submit"
                  className="bg-primary text-white rounded-md p-3 w-32"
                >
                  {isLoading ? (
